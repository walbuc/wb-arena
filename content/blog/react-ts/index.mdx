...
```ts
const isSomeDataType = (x: unkwown): x is MyDataType => {
  // run here a bunch of checks and if return true
  // will be cast to MyDataType
}
```
...
This is what react is doing under the hood for getting the type of the state
value in the reducer function.

```tsx
type ReducerState = ReturnType<typeof reducerfn>
```

Brand new features from Ts that are super useful for type safety:
...
[The context API is not a state management tool](https://blog.isquaredsoftware.com/2021/01/context-redux-differences/)
